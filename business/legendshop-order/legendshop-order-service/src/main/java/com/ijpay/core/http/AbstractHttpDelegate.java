...
/**
 *
 * TODO Legend-cloud 覆盖该类的目的是因为升级到了openJDK17,官方是因安全问题禁用了SSLv3, TLSv1, TLSv1.1等协议
 * TODO IJPay方个别方法依然使用SSLSocketFactoryBuilder.TLSv1，导致http调用报错
 * TODO 此处为兼容openJDK17，将协议升级为TLSv1.2
 * TODO 参考文档 https://learn.microsoft.com/zh-cn/java/openjdk/java-security，以及openJdk17 java security配置文件
 *
...
public abstract class AbstractHttpDelegate {
    ...
    public String post(String url, String data, InputStream certFile, String certPass) {
		log.info("##################### 进入自定义 post 请求 加密协议 TLSv12 #############");
        try {
            return HttpRequest.post(url)
                    .setSSLSocketFactory(SSLSocketFactoryBuilder
                            .create()
