 ...
 * @author legendshop
 */
@Component("APP_WX_PAY_REFUND")
public class WeChatAppRefundHandler extends AbstractWeChatRefundHandler {

	@Override
	public String getAppId() {
		WxConfig config = mapper.convertValue(super.sysParamsApi.getConfigDtoByParamName(SysParamNameEnum.WX_APP.name(), WxConfig.class).getData(), WxConfig.class);
	...
	public boolean isSupport(String payType, String paySource) {
		return PayTypeEnum.WX_PAY.name().equals(payType) && VisitSourceEnum.APP.name().equals(paySource);
	}
}
