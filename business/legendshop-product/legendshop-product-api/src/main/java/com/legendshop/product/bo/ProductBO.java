...
public class ProductBO implements Serializable {
	...
	private PreSellProductBO preSellProductBO;
	...
	public Boolean getActivitySelectFlag() {
		return !CollUtil.isEmpty(this.skuBOList);
	}


	public Integer getSkuBOListLength() {
		return CollUtil.isNotEmpty(getSkuBOList()) ? getSkuBOList().size() : 0;
	}

	public String getRemarks() {
		if (CollUtil.isEmpty(getSkuBOList())) {
			if (CollUtil.isEmpty(getIllegalSkuBOList())) {
				return "-";
			}
			return composeRemarks(getIllegalSkuBOList());
		}
		return composeRemarks(this.skuBOList);
	}
